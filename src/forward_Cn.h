// Forward modeling of geomagnetic Cn-responses 
...
#ifndef _forward_Cn_H
#define _forward_Cn_H

#include "utils.h"
#include <cmath>
#include <vector>

inline void forward_Cn(std::vector<double> radius,
...
{
   int M = radius.size();
   Cn_responses.resize(periods.size());
   for (int f=0; f<periods.size(); f++)
   {
      ...
      // Compute Cn-responses and convert to km
      Dcomplex Cn = -1.0/(II*omega*mu0*Y1)/1000.0; // to km
      Cn = std::conj(Cn); // convert from exp(-iwt) to exp(iwt)
      Cn_responses[f] = Cn;
   }
}
