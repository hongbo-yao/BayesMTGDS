...
int main(int argc, char* argv[])
{
   ...
   // Forward modeling
   std::vector<Dcomplex> Q_pred(n_periods);
   forward_Qn(radius, conductivity, SH_degree, periods, Q_pred);

   // Add gaussian random noise
   std::vector<Dcomplex> Q_pred_noise(n_periods);
   ...
   for (int i=0; i<n_periods; i++)
   {
      ...
      Q_pred_noise[i] = Q_pred[i]*(1+random_err*rand);
      d_err[i] = abs(Q_pred[i])*data_uncertainty;
   }
   std::ofstream out_stream("synthetic_Dst_Qn_data.txt");
   for (int i=0; i<n_periods; i++)
   {
      out_stream << std::setw(4) << std::setiosflags(std::ios::fixed) << std::setprecision(0) << SH_degree[i]
                 ...
                 << std::setw(15) << std::setiosflags(std::ios::fixed) << std::setprecision(6) << Q_pred_noise[i].real()
                 << std::setw(15) << std::setiosflags(std::ios::fixed) << std::setprecision(6) << Q_pred_noise[i].imag()
